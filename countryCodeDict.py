...
class CountryCodeDict(object):
    ...
    def lookup(self, ipAddress):
        ...
        return self.binary_search(intIP, 0, len(self.incLst) - 1)

    def binary_search(ip, imin, imax):
        if (imax < imin):
            return None
        imid = (imin + imax) / 2
        if (self.incLst[imid].compareWithLower(ip) == -1):
            return binary_search(ip, imin, imid - 1)
        elif (self.incLst[imid].compareWithLower(ip) == 1):
            return binary_search(ip, imid + 1, imax)
        else:
